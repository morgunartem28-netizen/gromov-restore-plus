...
class IpatoolClient:
    ...
    _TWO_FACTOR_HINT = (
        "Нужен код с iPhone — GROMOV не может прислать SMS за Apple.\n"
        "На телефоне: Настройки → [ваше имя] → Вход и безопасность → "
        "Получить код проверки.\n"
        "Введите 6 цифр вместе с паролем и сразу нажмите «Войти» "
        "(код живёт ~30 секунд). Не ждите уведомления: для этого входа "
        "Apple его часто не отправляет.\n"
        "Если кода нет — проверьте email/пароль: при опечатке Apple код не выдаёт."
    )
    ...
    def format_error(error_text: str, *, had_auth_code: bool = False) -> str:
        ...
        if "unexpected hex digit" in lower or "failed to unmarshal xml" in lower:
            return (
                "Apple отклонил служебный запрос (сеть/VPN/лимит), "
                "поэтому уведомление с кодом не уходит.\n"
                "Отключите VPN/прокси, подождите 2–5 минут.\n"
                "Затем на iPhone: Настройки → Apple ID → Вход и безопасность → "
                "Получить код проверки — и войдите с email, паролем и этими 6 цифрами."
            )
        if "something went wrong" in lower and "auth code" not in lower:
            if had_auth_code:
                return (
                    "Apple не принял код подтверждения.\n"
                    "На iPhone получите новый код: Настройки → Apple ID → "
                    "Вход и безопасность → Получить код проверки.\n"
                    "Введите его сразу (~30 сек) вместе с паролем."
                )
            return (
                "Apple не отправил код на устройство — для этого входа так бывает часто.\n"
                "Не ждите push. На iPhone: Настройки → Apple ID → "
                "Вход и безопасность → Получить код проверки.\n"
                "Введите email, пароль и 6 цифр одним заходом.\n"
                "Если не помогает: подождите несколько минут (лимит попыток) и отключите VPN."
            )
        ...
        if "badlogin" in lower or "incorrect" in lower:
            return (
                ...
                "Сначала проверьте пароль. Код берите на iPhone: "
                "Настройки → Apple ID → Получить код проверки — не из SMS/push."
            )
        return text
